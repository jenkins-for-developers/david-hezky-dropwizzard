...
* [Maven 3.6](https://maven.apache.org/) installation on "metal" with your JDK
...
* "Installing" application in the local Maven repository (no need to do this):
```bash
mvn install
```
...
```bash
java -jar ./target/demoapp.jar server ./env-develop.yml
```
